...
class Koan11 extends GroovyTestCase {
    ...
    void test02_MoreOnInsertingData() {
        ...
        Sql.withInstance('jdbc:h2:mem:groovyDb2', 'sa', '', 'org.h2.Driver') { db ->
            ...
            // ------------ START EDITING HERE ----------------------
            new File("src/test/groovy/org/groovykoans/koan11/cast.txt").eachLine {
                def name = it.split(" ")
                db.execute("insert into person(firstname, lastname) values (${name[0]}, ${name[1]})")
            }
            ...
            // ------------ START EDITING HERE ----------------------
            def names = new XmlSlurper().parse("src/test/groovy/org/groovykoans/koan11/cast2.xml")
            names.character.each{
                def name =  it.@actor.text().split(" ")
                db.execute("insert into person(firstname, lastname) values (${name[0]}, ${name[1]})")
            }
            ...
            person.add(firstname:'Tony', lastname:'Stark')
            // ------------ STOP EDITING HERE  ----------------------
    ...
    void test03_IteratingResults() {
        Sql.withInstance('jdbc:h2:mem:groovyDb3', 'sa', '', 'org.h2.Driver') { db ->
            ...
            def lastNameRose
            // ------------ START EDITING HERE ----------------------
            def name = db.firstRow('select lastname from PERSON where firstname = :firstname', [firstname: 'Rose'])
            lastNameRose = name.lastname
    ...
    void test04_UpdatingTables() {
        Sql.withInstance('jdbc:h2:mem:groovyDb4', 'sa', '', 'org.h2.Driver') { db ->
            ...
            // ------------ START EDITING HERE ----------------------
            def firstnames = db.rows("select firstname from person")
            firstnames.each {
                if(it.firstname.contains('a')){
                    db.execute("update person set firstname = 'Alf'")
                }
            }
