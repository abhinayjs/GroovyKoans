...
class Koan10 extends GroovyTestCase {
    ...
    void test01_XmlSlurpersReader() {
        ...
        def movieCount
        // ------------ START EDITING HERE ----------------------
        def movies = new XmlSlurper().parse('src/test/groovy/org/groovykoans/koan10/movies.xml')
        ...
        // ------------ START EDITING HERE ----------------------
        def list = movies.movie.title.findAll {
            it.text().toLowerCase().contains('the')
        }
        moviesWithThe = list.collect(){it.text()};
        ...
        // ------------ START EDITING HERE ----------------------
        movieIdsGreaterThan5 = movies.movie.findAll{it.@id.text().toInteger() > 5}.size()
    ...
    void test02_XmlSlurpersReader2() {
        ...
        // ------------ START EDITING HERE ----------------------
        def movies = new XmlSlurper().parse('src/test/groovy/org/groovykoans/koan10/movies.xml')
        def list = movies.movie.list().sort{node1, node2 ->
            def y1 = node1.year.text()
            def y2 = node2.year.text()
            return y1==y2 ? node1.title.text()<=>node2.title.text() : y1 <=> y2
        }
        sortedList = list.collect{it.title.text()}
    ...
    void test03_XmlMarkupBuilder1() {
        ...
        def xml = new MarkupBuilder(writer).html {
            body() {
                h1("title")
            }
        }
    ...
    void test06_AntBuilderChecksum() {
        ...
        // ------------ START EDITING HERE ----------------------
        def ant = new AntBuilder()
        ant.checksum(file: "${baseDir}/movies.xml", property: "res")
        actualChecksum = ant.project.properties.res
